  ...
  if (interaction.commandName === 'txt2img') {
    try {
      ...
      const negativePrompt = interaction.options.get('negative_prompt')
        ? interaction.options.get('negative_prompt').value
        : negative
      ...
      const amount = interaction.options.get('amount')
        ...
        : 1
      const sharePrompt = interaction.options.get('share_prompt')
        ? interaction.options.get('share_prompt').value
        : true
      ...
      interaction.followUp({
        content: `Seed: ${jsonInfo.seed.toString()}${
          sharePrompt ? '\nPrompt: ' + prompt : ''
        }`,
        files: images
      })
